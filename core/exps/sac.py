...
from core.policy.hybrid_sac import HybridSACPolicy
...
@EXPS.register_module("HYBRID_SAC", force=True)
class HybridSACEngine:
    ...
    def create_policy(self, config, train_envs, **kwargs):
        ...
        if cfg.alpha.auto:
            device = "cuda" if cfg.device == "dp" else cfg.device
            target_entropy_d = 0.98 * np.log(np.prod(action_space["retrace"].n))
            target_entropy_c = -np.prod(action_space["direction"].shape)

            log_alpha_d = torch.zeros(1, requires_grad=True, device=device)
            log_alpha_c = torch.zeros(1, requires_grad=True, device=device)
            alpha_optim = torch.optim.Adam([log_alpha_d, log_alpha_c], lr=cfg.alpha.lr)
            alpha = (target_entropy_d, target_entropy_c), (log_alpha_d, log_alpha_c), alpha_optim
        else:
            alpha = list(cfg.alpha.value)
            alpha[0] /= np.log(np.prod(action_space["retrace"].n))
