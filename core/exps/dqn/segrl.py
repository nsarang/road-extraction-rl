...
@EXPS.register_module("DQN_SEGRL", force=True)
class DQNSeqEngine:
    ...
    def create_policy(self, config, train_envs, **kwargs):
        ...
        segmodel = locate(cfg.seg.name)(**cfg.seg.args)
        state_dict = torch.load(cfg.seg.checkpoint)["state_dict"]
        ...
        if cfg.device == "dp":
            ...
            src_device_policy = ids[0]

            model = TSDP(model, device_ids=ids).cuda(src_device_policy)
            segmodel = TSDP(segmodel, device_ids=ids[::-1]).cuda(ids[-1])
        else:
            model.to(device)
            segmodel.to(device)
        ...
        def preprocess_fn(**kwargs):
            kwargs = dict(Batch(kwargs))

            if ("obs" in kwargs) or ("obs_next" in kwargs):
                key = "obs" if "obs" in kwargs else "obs_next"
                inputs = kwargs[key]
                obs = inputs["obs"]
                rgb = obs[:, :3]
                with torch.no_grad():
                    output = segmodel(rgb)[0]
                    masks = [F.softmax(mask, dim=1)[:, 1] for mask in output]
                    masks = torch.stack(masks, dim=1).cpu().numpy()
                obs = np.concatenate((obs, masks), axis=1)
                # crop
                _, _, rh, rw = obs.shape
                cw, ch = cfg.seg.center_crop
        ...
        if cfg.retrace_prob:
            print("Retrace rand enabled.")
            def exp_noise(self, act, batch):
                if isinstance(act, np.ndarray) and not np.isclose(self.eps, 0.0):
                    if hasattr(batch.obs, "mask"):
                        raise RuntimeError
                    bsz = len(act)
                    n_act = self.max_action_num
                    rand_mask = np.random.rand(bsz) < self.eps
                    probs = [cfg.retrace_prob, *([(1 - cfg.retrace_prob) / (n_act - 1)] * (n_act - 1))]
                    rand_act = np.random.choice(np.arange(0, n_act), size=bsz, p=probs)
                    act[rand_mask] = rand_act[rand_mask]
                return act
            
            policy.exploration_noise = types.MethodType(exp_noise, policy)

        self.register(policy=policy, optim=optim, finetune_callback=finetune_callback, preprocess_fn=preprocess_fn)

    def create_collector(self, config, train_envs, test_envs, policy, preprocess_fn, **kwargs):
        ...
        test_collector = Collector(policy, test_envs, exploration_noise=True, preprocess_fn=preprocess_fn)
    ...
    def run_trainer(self, config, train_collector, test_collector, policy, optim, finetune_callback, **kwargs):
        ...
        logger = BasicLogger(writer, train_interval=10, update_interval=10)
