...
class BasePolicy(ABC, nn.Module):
    ...
    * :meth:`~tianshou.policy.BasePolicy.update`: the main interface for training, \
    ...
    def compute_nstep_return(
        ...
        if isinstance(gamma, (np.floating, float)):
    ...
    def _compile(self) -> None:
        ...
        _nstep_return(f64, b, f32.reshape(-1, 1), i64, f64[:1], 1)
...
def _nstep_return(
    ...
    gamma_buffer = np.ones((n_step + 1, gamma.shape[0]))
    for i in range(1, n_step + 1):
        gamma_buffer[i] = gamma_buffer[i - 1] * gamma
    if target_q.ndim == 1:
        target_q = target_q.reshape(-1, 1)
    bsz, nhoriz = target_q.shape
    assert nhoriz == gamma.shape[0]

    returns = np.zeros(target_q.shape)
    gammas = np.full(bsz, n_step)
