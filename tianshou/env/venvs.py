...
class BaseVectorEnv(gym.Env):
    ...
    def render(self, id: Optional[Union[int, List[int], np.ndarray]] = None, **kwargs: Any) -> List[Any]:
        ...
        return [self.workers[j].render(**kwargs) for j in id]

    def save(self, id: Optional[Union[int, List[int], np.ndarray]] = None, **kwargs: Any) -> None:
        id = self._wrap_id(id)
        self._assert_is_not_closed()
        if self.is_async and len(self.waiting_id) > 0:
            raise RuntimeError(
