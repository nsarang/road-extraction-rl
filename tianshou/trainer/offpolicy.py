...
def offpolicy_trainer(
    ...
    last_rew, last_len, last_rew_avg = 0.0, 0, 0.0
    ...
    for epoch in range(1 + start_epoch, 1 + max_epoch):
        ...
        with tqdm.tqdm(
            ...
            while t.n < t.total:
                ...
                print("Train collection took", f"{int((time.time() - s) * 1000)} ms")
                ...
                last_rew_avg = result["rew_avg"] if "rew_avg" in result else last_rew_avg
                data = {
                    "env_step": str(env_step),
                    "rew_avg": f"{last_rew_avg:.2f}",
                    "rew": f"{last_rew:.2f}",
                ...
                if result["n/ep"] > 0:
                    if test_in_train and stop_fn and stop_fn(epoch, result):
                        test_result = test_episode(
                            policy, test_collector, test_fn, epoch, episode_per_test, logger, env_step
                        )
                        if stop_fn(epoch, test_result):
                            if save_fn:
                                save_fn(policy, epoch, test_result)
                ...
                for i in range(round(update_per_step * result["n/st"])):
                    gradient_step += 1
                    losses = policy.update(batch_size, train_collector.buffer, logger=logger, step=t.n)
                    ...
                    t.set_postfix(**data)
                print("Optimization took", f"{int((time.time() - s) * 1000)} ms")
            if t.n <= t.total:
                t.update()
        # test
        test_result = test_episode(
            policy, test_collector, test_fn, epoch, episode_per_test, logger, env_step, reward_metric
        )
        ...
        if stop_fn and stop_fn(epoch, test_result):
            break
    return gather_info(
