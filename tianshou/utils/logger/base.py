...
class BaseLogger(ABC):
    ...
    def log_train_data(self, collect_result: dict, step: int) -> None:
        ...
        if collect_result["n/ep"] > 0:
            ...
            collect_result["rew_avg"] = collect_result["rews/avg"].mean()
            extra_logs = {
                k: np.mean(v)
                for k, v in collect_result.items()
                if k not in ["n/ep", "n/st", "rews", "lens", "rews/avg", "idxs"]
            }
            if step - self.last_log_train_step >= self.train_interval:
                log_data = {
                    ...
                    "train/reward_avg": collect_result["rew_avg"],
                    **{f"train/{k}": v for k, v in extra_logs.items()},
                }
    ...
    def log_test_data(self, collect_result: dict, step: int) -> None:
        ...
        rews, lens, rews_avg = collect_result["rews"], collect_result["lens"], collect_result["rews/avg"]
        rew, rew_std, len_, len_std, rew_avg = rews.mean(), rews.std(), lens.mean(), lens.std(), rews_avg.mean()
        extra_logs = {
            k: np.mean(v)
            for k, v in collect_result.items()
            if k not in ["n/ep", "n/st", "rews", "lens", "rews/avg", "idxs"]
        }
        collect_result.update(rew=rew, rew_std=rew_std, len=len_, len_std=len_std, rew_avg=rew_avg, **extra_logs)
        if step - self.last_log_test_step >= self.test_interval:
            log_data = {
                ...
                "test/reward_avg": rew_avg,
                **{f"test/{k}": v for k, v in extra_logs.items()},
            }
