...
class Actor(nn.Module):
    ...
    def __init__(
        ...
        self.last = head or MLP(input_dim, self.output_dim, hidden_sizes, device=self.device)
...
class Critic(nn.Module):
    ...
    def __init__(
        ...
        self.last = head or MLP(input_dim, 1, hidden_sizes, device=self.device)
    ...
    def forward(
        ...
        if a is not None:
            a = to_torch_as(a, logits).flatten(1)
            logits = torch.cat([logits, a], dim=1)
...
class ActorProb(nn.Module):
    ...
    def __init__(
        ...
        self.output_dim = int(np.prod(action_shape))
        input_dim = getattr(preprocess_net, "output_dim", preprocess_net_output_dim)
        self.mu = head or MLP(input_dim, self.output_dim, hidden_sizes, device=self.device)
